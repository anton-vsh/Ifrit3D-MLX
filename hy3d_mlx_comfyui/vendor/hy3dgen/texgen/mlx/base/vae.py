...
class Attention(nn.Module):
    ...
    def __call__(self, x):
        ...
        queries = self.query_proj(y).reshape(B, H * W, 1, C)
        keys = self.key_proj(y).reshape(B, H * W, 1, C)
        values = self.value_proj(y).reshape(B, H * W, 1, C)

        y = mx.fast.scaled_dot_product_attention(
            queries, keys, values, scale=C ** -0.5
        )
        y = y.reshape(B, H, W, C)
...
class Autoencoder(nn.Module):
    """The autoencoder for latent diffusion."""
