...
class Basic2p5DTransformerBlock(torch.nn.Module):
    ...
    def forward(
        ...
        if "r" in mode and self.use_ra:
            ...
            # Handle varying output shapes across diffusers versions:
            # Ref attention uses only albedo features, broadcast result to all PBR channels
            if attn_output.ndim == 3:
                attn_output = attn_output.unsqueeze(1).expand(-1, N_pbr, -1, -1)
            elif attn_output.shape[1] != N_pbr:
                attn_output = attn_output[:, :1].expand(-1, N_pbr, -1, -1)
