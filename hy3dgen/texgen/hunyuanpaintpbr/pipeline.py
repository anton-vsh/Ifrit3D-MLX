        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if hasattr(self.unet, '_mlx_hybrid'):
                    unet_sample = latent_model_input.to(dtype=self.unet.dtype) if hasattr(latent_model_input, 'to') else latent_model_input
                    unet_prompt_embeds = prompt_embeds.to(dtype=self.unet.dtype) if hasattr(prompt_embeds, 'to') else prompt_embeds
                else:
                    unet_sample = latent_model_input.to(device=self.unet.device, dtype=self.unet.dtype) if hasattr(latent_model_input, 'to') else latent_model_input
                    unet_prompt_embeds = prompt_embeds.to(device=self.unet.device, dtype=self.unet.dtype) if hasattr(prompt_embeds, 'to') else prompt_embeds
